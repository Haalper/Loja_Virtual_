...
@ControllerAdvice //indica que essa classe vai interceptar exceções
public class ControleExcecoes extends ResponseEntityExceptionHandler {
	
	@ExceptionHandler(ExceptionMentoriaJava.class) //exceção customizada pela classe ao lado)
	public ResponseEntity<Object> handleExceptionCustom (ExceptionMentoriaJava ex) {
		
		ObjetoErroDTO objetoErroDTO = new ObjetoErroDTO();
		
		objetoErroDTO.setError(ex.getMessage()); //mostra a mensagem de exceção pro usuário, configurada no endpoint obterAcesso do AcessoController
	...
	@ExceptionHandler({DataIntegrityViolationException.class, 
			ConstraintViolationException.class, SQLException.class}) //exceções mais comuns no bd
